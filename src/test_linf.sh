...
do
for max in 1000 5000
do  
python main.py --fd-eta 0.1 --max-queries $max --image-lr 0.005 --mode linf --online-lr 100 --exploration 0.01 --epsilon $epsilon --batch-size 500 --gradient-iters 1  --resume_path checkpoint/$resume_path | tee log/$epsilon/max_queries_$max\_$resume_path.txt
done
done
done
